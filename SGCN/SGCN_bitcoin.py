...
def load_and_clean_data(file_path='soc-sign-bitcoinalpha.csv'):
    ...
    df.dropna(subset=['source', 'target', 'rating'], inplace=True)
    
    # Create sign from rating
    df['sign'] = np.sign(df['rating'])
...
def perform_eda(df):
    ...
    sns.histplot(df['rating'], bins=21, kde=False)
    plt.title('Distribution of Ratings (-10 to +10)')
    plt.xlabel('Rating')
    plt.ylabel('Count')
    ...
    plt.title('Distribution of Signs (1: Positive, -1: Negative)')
    plt.savefig('sign_distribution.png')
    plt.close()


    
    # Time distribution (if applicable)
    if 'time' in df.columns:
        ...
        plt.title('Distribution of Edge Creation Times')
        plt.xlabel('Time')
        plt.ylabel('Count')
...
class SignedGCN(nn.Module):
    def __init__(self, num_nodes, in_channels, hidden_channels, out_channels, dropout=0.5):
        ...
        # Here we use learnable as in many link prediction tasks without rich features.
...
def train_and_evaluate(df, epochs=500, lr=0.01, patience=30):
...
if __name__ == "__main__":
    try:
        df = load_and_clean_data('soc-sign-bitcoinalpha.csv')
        perform_eda(df)
        train_and_evaluate(df)
    except FileNotFoundError:
        print("Error: 'soc-sign-bitcoinalpha.csv' not found. Please upload the dataset.")
